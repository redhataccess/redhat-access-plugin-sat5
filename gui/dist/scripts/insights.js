(function() {
    ...
    RHA_INSIGHTS.UTILS.isOnInsightsEnabledPage = function() {
        var response = false;
        if (RHA_INSIGHTS.UTILS.isOnSystemListPage() || RHA_INSIGHTS.UTILS.isOnSystemDetailsPage() || RHA_INSIGHTS.UTILS.isOnSystemOverviewPage() || RHA_INSIGHTS.UTILS.isOnAdminPage() || RHA_INSIGHTS.UTILS.isOnInsightsOverviewPage() || RHA_INSIGHTS.UTILS.isOnSystemTabPage() || RHA_INSIGHTS.UTILS.isOnHelpPage()) {
            response = true;
        }
        return response;
    };
    ...
    RHA_INSIGHTS.UTILS.isOnSystemTabPage = function() {
        var response = false;
        if (RHA_INSIGHTS.UTILS.isOnPage(SAT5_ROOT_URLS.SYSTEMS) || RHA_INSIGHTS.UTILS.isOnPage(SAT5_ROOT_URLS.SSM) || RHA_INSIGHTS.UTILS.isOnPage(SAT5_ROOT_URLS.ACTIVATIONKEYS) || RHA_INSIGHTS.UTILS.isOnPage(SAT5_ROOT_URLS.PROFILES) || RHA_INSIGHTS.UTILS.isOnPage(SAT5_ROOT_URLS.KICKSTART) || RHA_INSIGHTS.UTILS.isOnPage(SAT5_ROOT_URLS.KEYS)) {
            response = true;
        }
        return response;
    };
    RHA_INSIGHTS.UTILS.isOnSystemDetailsPage = function() {
        var response = false;
        if (RHA_INSIGHTS.UTILS.isOnPage(SYSTEM_DETAILS_PAGE_URLS.OVERVIEW) || RHA_INSIGHTS.UTILS.isOnPage(SYSTEM_DETAILS_PAGE_URLS.PROPERTIES) || RHA_INSIGHTS.UTILS.isOnPage(SYSTEM_DETAILS_PAGE_URLS.HARDWARE) || RHA_INSIGHTS.UTILS.isOnPage(SYSTEM_DETAILS_PAGE_URLS.MIGRATE) || RHA_INSIGHTS.UTILS.isOnPage(SYSTEM_DETAILS_PAGE_URLS.NOTES) || RHA_INSIGHTS.UTILS.isOnPage(SYSTEM_DETAILS_PAGE_URLS.CUSTOM_INFO) || RHA_INSIGHTS.UTILS.isOnPage(SYSTEM_DETAILS_PAGE_URLS.INSIGHTS)) {
            response = true;
        }
        return response;
    };
    ...
    RHA_INSIGHTS.UTILS.isOnSystemOverviewPage = function() {
        ...
        if (RHA_INSIGHTS.UTILS.isOnPage(SYSTEM_PAGE_URLS.SYSTEM_OVERVIEW) && !showGroups) {
            response = true;
        }
        return response;
    };
    RHA_INSIGHTS.UTILS.showGroups = function() {
        ...
        if (showGroupsIndex !== -1) {
            var value = "";
            value = query.substring(showGroupsIndex, 16);
            //16 is length of showgroups=false
    ...
    RHA_INSIGHTS.UTILS.isOnSystemListPage = function() {
        if ((RHA_INSIGHTS.UTILS.isOnPage(SYSTEM_PAGE_URLS.SYSTEMS) || RHA_INSIGHTS.UTILS.isOnPage(SYSTEM_PAGE_URLS.PHYSICAL) || RHA_INSIGHTS.UTILS.isOnPage(SYSTEM_PAGE_URLS.OUT_OF_DATE) || RHA_INSIGHTS.UTILS.isOnPage(SYSTEM_PAGE_URLS.REQUIRING_REBOOT) || RHA_INSIGHTS.UTILS.isOnPage(SYSTEM_PAGE_URLS.UNGROUPED) || RHA_INSIGHTS.UTILS.isOnPage(SYSTEM_PAGE_URLS.INACTIVE) || RHA_INSIGHTS.UTILS.isOnSystemOverviewPage()) && !RHA_INSIGHTS.UTILS.showGroups()) {
            return true;
        } else {
            return false;
        }
    ...
    if (RHA_INSIGHTS.UTILS.isOnInsightsEnabledPage()) {
        $.get("/redhat_access/config/general").done(function(insightsConfig) {
            ...
            if (insightsConfig.enabled || RHA_INSIGHTS.UTILS.isOnAdminPage()) {
                if (RHA_INSIGHTS.UTILS.isOnSystemListPage()) {
                    $("#spacewalk-content").prepend('<div id="rha-insights-sat5-loading-alert" class="alert alert-info">' + "Loading Red Hat Access Insights..." + "</div>");
                }
                $.ajax({
                    dataType: "script",
                    cache: true,
